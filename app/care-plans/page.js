      ...
      <Section title="Care Plans & Pricing">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {PLANS.map((p) => (
            <div
              key={p.name}
              className="group relative rounded-2xl border border-teal/20 bg-gradient-to-b from-white to-teal-50/30 p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-md"
            >
              {p.popular && (
                <div
                  className="absolute -top-3 right-3 rounded-full bg-[#167a7a] text-white text-xs px-3 py-1 shadow-soft"
